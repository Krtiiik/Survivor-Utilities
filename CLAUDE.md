...
4. **Config tab** (`config_editor/`; the Timesheet-only settings — Timesheet Teams count, Time, Activities — share one `TimesheetEditor` sub-tab) — edits an in-memory `Config` and only writes `config.json` (via `core.config.save_config`) once `core.config.validate` passes.
...
This is the only module with real algorithmic complexity — a constraint-satisfaction model, not a heuristic. The CP-SAT model was reworked from the original script so the solver itself decides how oversize Kruhy are split, instead of splitting them up front.
...
- The solver tries every value of `Possible Teams sizes` from the config (`compute_distributions`), building and solving a fresh CP-SAT model per size (`compute_teams_distribution`) with `config.teams_count` (= `len(Teams names)`) Teams available. It keeps all resulting `Solution`s; feasible or optimal ones are shown as selectable candidates in the Distribution tab. There is no Team-count search dimension, because the objective already minimizes Teams used, so the solver picks the count itself. `Solution.num_teams` is the number of Teams available, not the number used (`len(solution.distribution)`).
- Model: a bool per (Kruh, Team) with exactly one true, and a bool per (Kruh, Team, Subteam) "has a part here". A Kruh that fits into a Subteam gets exactly one part. A Kruh larger than the candidate Subteam size gets exactly `ceil(count / max_subteam_size)` parts, all in its own Team, at most one per Subteam. For those Kruhy only, an int variable holds each part's size: the parts sum to the Kruh's count, and each part is at least `config.min_split_part_size` (`"Min split part size"`, clamped down to `count // parts` so it can never make a Kruh infeasible). A Kruh bigger than a whole Team (`num_subteams * max_subteam_size`) makes that combination INFEASIBLE.
- Symmetry breaking: Team sizes are non-increasing over Team index, and Subteam sizes are non-increasing within each Team. Teams are interchangeable, and so are Subteams within a Team.
- Objective: number of Teams used is strictly dominant. Below it sits a weighted blend of distinct Obory per Team, Team-size spread (max − min over used Teams) and Subteam-size spread (max − min over every Subteam of every used Team, where an empty Subteam counts as 0). The weights `OBORY_WEIGHT` / `TEAM_SPREAD_WEIGHT` / `SUBTEAM_SPREAD_WEIGHT` live in `core/models.py`. `Solution.score()` recomputes the same blend from a (possibly hand-edited) distribution to rank candidates in the Distribution tab.
- The parts of a split Kruh are returned as synthetic sub-Kruhy with IDs of the form `100*kruh_id + part_index`. Every used Team is emitted with all of its Subteams, including empty ones. `core.models.format_kruh_label` renders these split IDs back as `"{id}[{letter}]"` for both the GUI grid and Excel export. The `DistributionGrid` shows a soft ⚠ warning (not a hard block) if a user manually drags split-Kruh siblings into different Teams — full manual override is intentional.
- Each solve stops after `config.solver_time_limit` seconds (`"Solver time limit"`, default 30, passed to `compute_teams_distribution` as `time_limit`) with the best solution found. It can also be cancelled early via a `CpSolverSolutionCallback` (`_CancelCallback`) driven by the GUI's Cancel button; the outer per-combination loop also checks the same `should_cancel` between combinations.
...
`"Subteams count"`/`"Activities count"` were removed from `config.json` — those counts are now derived from `len(config["Subteams"])`/`len(config["Activities"])` (see `Config.subteams_count`/`activities_count` in `core/config.py`). `"Teams count"` and `"Possible Teams counts"` were removed too (2.0.0): `Config.teams_count` is `len(config["Teams names"])`, the most Teams the Distribution solver may use. Older configs still load; the keys are ignored. The Timesheet has its own `"Timesheet Teams count"` (`Config.timesheet_teams_count`, the first N names). It must be ≤ both the number of Teams names and the number of Activities, since each Team starts on a different Activity. When missing it defaults to the largest valid value. When it's below the number of Activities, the leftover starting slots become `team_empty` cells showing `EMPTY_SLOT_SYMBOL` (`∅`). `"Activity duration"` is HH:MM (matches the README and the code's actual runtime behavior).
